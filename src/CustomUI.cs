...
namespace SFSZoomKeys
{
    ...
    public static class CustomUI
    {
        public static NumberInput CreateNumberInput(Transform parent, int width, int height,
            double defaultVal, double min, double max, int charLimit = 100, UnityAction<string> onChange = null,
            int posX = 0,
            int posY = 0)
        {
            var ToReturn = new NumberInput
            {
                textInput = Builder.CreateTextInput(parent, width, height, posX, posY,
                    defaultVal.ToString(CultureInfo.InvariantCulture), onChange),
